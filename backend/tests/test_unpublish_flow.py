...
def test_unpublish_sync() -> None:
    ...
    create = client.post(
        "/api/v1/kb/create",
        headers=headers,
        json={"title": "QAT", "content_md": "## Abstract\nQAT details"},
    )
    kb_id = create.json()["id"]

    client.post(f"/api/v1/kb/{kb_id}/publish", headers=headers)
    unpublish = client.post(f"/api/v1/kb/{kb_id}/unpublish", headers=headers)
    assert unpublish.status_code == 200, unpublish.text

    vector_dir = Path(__file__).resolve().parents[1] / "vector_store"
    assert not (vector_dir / "kb.index.faiss").exists()
